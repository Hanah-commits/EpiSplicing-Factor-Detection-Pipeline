...
junctions = pd.read_csv('0_Files/majiq_junctions_control.csv', delimiter='\t')
...
for length in flank_lens:
        ...
        if length < 200:
                file = "majiq_flanks" + str(length) + "_control.bed"
        ...
        flanks = pd.read_csv('0_Files/majiq_flanks' + str(length) + '_control.bed', delimiter='\t', header=None)
...
flank_jns_group[['seqid', 'start', 'stop']].drop_duplicates().to_csv('0_Files/filtered_flanks_control.bed', index=False, sep='\t', header=False)

# Get dPSI values of filtered flanks
flank_jns_group[['flanks', 'gene_id', 'mean_dpsi_per_lsv_junction']].drop_duplicates().to_csv('0_Files/Filtered_dPSI_control.csv', index=False, sep='\t')
