...
grep -E "transcript_support_level=[12345]" $gff3 | grep "protein_coding" | awk -F'\t' -v OFS='\t' '$3 == "exon" {match($9, /gene_id=([^;]+)/, gene_id); print $1, $4, $5, "Exon", ".", $7, gene_id[1];}' | sort | uniq > ${1}_0_Files/all_exons.bed
...
# get TSS exons
grep -vFf ${1}_0_Files/exon_coords.bed ${1}_0_Files/all_exons.bed > ${1}_0_Files/TSS_exons.bed

# drop exons that overlap with TSS exons (longer/shorter version of TSS exons)
bedtools intersect -v -a ${1}_0_Files/exon_coords.bed -b ${1}_0_Files/TSS_exons.bed > ${1}_0_Files/exon_coords_temp.bed && mv ${1}_0_Files/exon_coords_temp.bed ${1}_0_Files/exon_coords.bed 

## For transcripts with TSL 1-3:

# get all exons
grep -E "transcript_support_level=[123]" $gff3 | grep "protein_coding" | awk -F'\t' -v OFS='\t' '$3 == "exon" {match($9, /gene_id=([^;]+)/, gene_id); print $1, $4, $5, "Exon", ".", $7, gene_id[1];}' | sort | uniq > ${1}_0_Files/all_exons.bed

# get non-TSS exons
grep -Ff ${1}_0_Files/exon_coords.bed ${1}_0_Files/all_exons.bed > ${1}_0_Files/exon_coords_temp.bed && mv ${1}_0_Files/exon_coords_temp.bed ${1}_0_Files/exon_coords.bed 
