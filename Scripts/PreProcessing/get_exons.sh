...
gff3=$(jq -r ".[$field1]" "$json_file")
fasta=$(jq -r ".[$field2]" "$json_file")
...
awk 'BEGIN {OFS="\t"} {
    ...
    print
}' 0_Files/gencode_TSS.tsv > 0_Files/TSS.tsv
...
bedtools intersect -wa -a 0_Files/exons_flanked.bed -b 0_Files/TSS.bed -s -v > 0_Files/filtered_flanked_exons.bed
