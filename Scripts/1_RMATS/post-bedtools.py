...
for exon_type in exon_types:
        for type in event_types:
                print(exon_type, type)

                junctions = pd.read_csv(f'0_Files/{type}_exons_{exon_type}.csv', delimiter='\t')
                ...
                for length in flank_lens:
                        ...
                        file = f'{type}_flanks{length}_{exon_type}.bed'
                ...
                #         # adjust flank length to 200bp +- without exceeding chromosome bounds
                        ...
                        flanks = pd.read_csv('0_Files/'+ file, delimiter='\t', header=None)
                        ...
                        flanks.columns = ['chr', 'start', 'stop', 'exon_coord0', 'flanks']

                        junctions['index'] = junctions.index
                        flank_jns = pd.merge(junctions, flanks, on=['exon_coord0', 'chr'])

                        flank_jns.sort_values(['index'], inplace=True)
                        flank_jns = flank_jns.reset_index(drop=True)
                        del (flank_jns['index'])
                        flank_jns['dpsi_'+str(length)] = pd.to_numeric(flank_jns['IncLevelDifference'])
                ...
                flank_jns_group.columns = ['flanks', 'IncLevelDifference']
                ...
                flank_jns_group = flank_jns_group[flank_jns_group['IncLevelDifference'] == flank_jns_group['max_dPSI']]
                ...
                flank_jns_group = flank_jns_group[['GeneID', 'geneSymbol', 'chr', 'exon_coord0', 'IncLevelDifference',
                        'FDR', 'flanks', 'start', 'stop', 'strand']]
                

                # # Get all filtered flanks
                flank_jns_group.drop_duplicates().to_csv(f'0_Files/all_{type}_flanks_{exon_type}.csv', sep='\t', index=False)

                # # Get new annotation file with flanks (to annotate MANorm peaks)
                flank_jns_group[['chr', 'start', 'stop']].drop_duplicates().to_csv(f'0_Files/filtered_{type}_flanks_{exon_type}.bed', index=False, sep='\t', header=False)

                # # Get dPSI values of filtered flanks
                flank_jns_group[['chr', 'flanks', 'geneSymbol', 'IncLevelDifference']].drop_duplicates().to_csv(f'0_Files/Filtered_dPSI_{type}_{exon_type}.csv', index=False, sep='\t')
