...
def significane_test(test):
    ...
    if test == 'binomial':
        epi_pvals = pd.read_csv('0_Files/Post-processing/pvals_rbpepi.csv', delimiter='\t')[sfs]
    ...
    with open('0_Files/Post-processing/enriched_epi.txt', 'w') as f:
        for rbp in enriched_epi:
            f.write(f"{rbp}\n")

    with open('0_Files/Post-processing/enriched_nonepi.txt', 'w') as f:
