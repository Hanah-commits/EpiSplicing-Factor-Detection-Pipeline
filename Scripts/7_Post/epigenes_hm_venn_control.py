...
def indiv_hms():
    ...
    for elem in hm_cols:
        ...
        flanks[flanks['gene_id'].isin(hm_epigenes[i])].to_csv('0_Files/dPSI_Mval_epi_' + hm + '_control.csv', sep='\t', index=False)
...
def venn_plot(values, labels, info):
    ...
    plt.title(title)
    plt.savefig('0_Files/hm_overlap_' + info + '_control.png')
