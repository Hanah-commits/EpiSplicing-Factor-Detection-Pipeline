...
def make_hm_plots(hm, both_hm_flanks):
    ...
    Path(f'0_Files/DEXSEQ/{hm}/').mkdir(parents=True, exist_ok=True)
    ...
    with open(f'0_Files/DEXSEQ/{hm}_truepos_epigenes.txt', 'w') as f:
        for line in true_genes:
            f.write("%s\n" % line)

    both_hm_flanks = both_hm_flanks[both_hm_flanks['geneSymbol'].isin(true_genes)]
    both_hm_flanks.to_csv('0_Files/DEXSEQ/dPSI_Mval_epi_' + hm + '_dexseq.csv', sep='\t', index=False)
    ...
    for gene in true_genes:
        gene_df = both_hm_flanks[both_hm_flanks['geneSymbol'] == gene]
        corr_plot(gene_df, gene, hm, path=f'0_Files/DEXSEQ/{hm}/')
...
def plot_venn():
    ...
    for hm in hms:
        with open(f'0_Files/DEXSEQ/{hm}_truepos_epigenes.txt') as file:
    ...
    plt.title(title)
    plt.savefig('0_Files/DEXSEQ/hm_overlap_' + info + '.png')
