...
data <- read.table('0_Files/TSS_exons.bed', header = FALSE, sep = "\t", stringsAsFactors = FALSE)
colnames(data) <- c('chr', 'start', 'stop', 'feature', 'meta', 'strand','gene_id')
# Extract Ensembl IDs
data$ensembl_ids = sub("\\..*$", "", data$gene_id) # ENSG00000183878.15 -> ENSG00000183878
...
merged_data$external_gene_name[missing_or_whitespace] <- merged_data$gene_id[missing_or_whitespace]

merged_data_ids = unique(merged_data$gene_id)
...
write.table(merged_data, file = "0_Files/Post-processing/TSS.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
